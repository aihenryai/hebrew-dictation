...
- **`model.rs`** extension: the `piper.exe` binary and the Hebrew voice `.onnx` file are downloaded on demand, **not bundled in the NSIS installer** — same reasoning as the existing whisper models (keeps installer size down, matches the "download a model" UX users already know from this app).
- **New Tauri commands:**
