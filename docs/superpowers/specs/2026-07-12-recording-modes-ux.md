...
```
- הכותרות (`הקלטה רגילה` / `פגישות`) הן מפרידים ויזואליים; **ללא** קידומת "קבוצה א׳/ב׳" (רעש מיותר).
...
### 3.2 בורר ענן/מקומי — מותנה-הקשר
- ה-`batch-mode-cards` הקיים (☁ מהיר—ענן / 🔒 פרטי—מכשיר, `App.tsx:2406-2427`) מוצג **רק** כאשר `recordSource ∈ {mic, system}`.
- בשני מצבי הפגישה הוא **לא נרנדר כלל** — הכרטיס עצמו קובע את המנוע (עם-זיהוי = ענן כפוי, פרטית = מקומי כפוי). זה מסלק את הסתירה.
- הערת השקיפות הקיימת ("שיחה מתומללת תמיד בענן…", `App.tsx:2476-2483`) הופכת למיותרת כבורר נפרד — המידע עובר ל**שורת-התיאור** של כרטיס "עם זיהוי דוברים". מוחקים את פסקת ה-note הנפרדת.

### 3.3 סדר הרינדור במסך
1. בוררי המקור (שתי הקבוצות) — הבחירה הראשית.
2. בורר ענן/מקומי — מתחת, מותנה (§3.2).
...
- **Cancel ל-CallLocal:** עובר במסלול `cancel_batch_recording` / `stop_batch_recording_to_file`. ⚠️ יש לוודא שהביטול מנקז **גם** את ה-system recorder (הבאג הקריטי `af30355` מ-`CallCloud` — Cancel שהשאיר את thread ה-loopback רץ + נעל starts עתידיים). `CallLocal` = `(true,true)` כמו `CallCloud`, אז אותו סיכון בדיוק → הבדיקה חייבת לכסות אותו.
...
- **`lib.rs`** — ענף `CallLocal` ב-`start_batch_recording` (guard מודל) + ב-`stop_batch_recording_to_file`/`stop_recorder_for_source` (ניקוז-כפול + `mix_to_mono`); שינוי-שם `Call`→`CallCloud` בכל ההתאמות (`matches!`, `unreachable!` וכו'). `stop_call_recording` ללא שינוי לוגי מעבר לשם.
- **`model.rs` / `settings.rs`** — **קריאה בלבד** (`is_model_downloaded`, `preferred_model`); ללא שינוי.
- **Frontend `App.tsx`** — טיפוס `RecordingSource` (`mic|system|callcloud|calllocal`); בורר המקורות (2 קבוצות + כותרות + 2 כרטיסי פגישה); רינדור מותנה של `batch-mode-cards`; מחיקת פסקת ה-note הנפרדת; `handleStopBatchRecord`: `isCall`→`isCallCloud` (רק `callcloud` במסלול האינליין), `CallLocal` במסלול-הקובץ עם `mode="local"` כפוי; `BatchResult.isCall` נשאר `true` רק ל-`callcloud` (מזין סגנון SRT — `CallLocal` = `Diarization`/מונו).
