...
- **דימום לפגישות:** כשנבחר מקור פגישה (`callcloud`/`calllocal`) הטוגל מקבל class `is-disabled` (opacity 0.4 + `pointer-events:none`) והכפתורים `disabled` — כי הפגישה קובעת את המנוע בעצמה. זה מסלק את הסתירה **בלי** להסתיר את הבורר (הנרי העדיף טוגל-מעומעם על היעלמות/הופעה). ⚠️ הערת-כותרת מפורשת ("· קובעות מנוע בעצמן") נוסתה **ונדחתה ע"י הנרי כמיותרת** (2026-07-13) — שורות-התיאור של הכרטיסים ("מתומלל בענן" / "נשאר במחשב") + הטוגל המעומעם כבר מעבירים את זה. אל תחזיר אותה.
- **שורות-התיאור של כרטיסי הפגישה (סימטריות):** ענן = `אתם + הצד השני, כל אחד בנפרד · מתומלל בענן` · מקומי = `אתם + הצד השני יחד, ללא הפרדה לדוברים · נשאר במחשב`.
