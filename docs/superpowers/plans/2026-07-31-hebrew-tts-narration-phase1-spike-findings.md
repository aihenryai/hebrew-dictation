...
> **Addendum 2 (2026-08-15) — the engine changed, and this finding no longer applies.** The narration engine moved from piper's nikud-based phonemizer to **Phonikud** (stress-marked IPA) with the `michael` voice, after Henry judged the old intonation unusable. Re-measuring this exact sentence against the new engine: **4/4 exact ASR round-trips** (vs 0/6 at length_scale 1.0 and 2/6 at 1.18 on the old one). **The short-utterance instability is gone** — it was a symptom of the missing stress information, not an inherent property of small Hebrew TTS. The *mixed-content* limitation below is unchanged and still reproduces every time (0/4 exact on "הורדתי את ChatGPT 5 אתמול."), because the phonemizer is Hebrew-only. The UI copy was updated to warn only about the limitation that still exists.

> **Addendum 1 (2026-08-14, after Phase 1 shipped and was manually tested):** the instability is real but **partly a speed artifact**, which this spike missed by only tuning `noise_scale`/`noise_w_scale` and never varying `length_scale`. Re-measured with the same sentence, same ASR round-trip, 6 takes per setting: at `length_scale` **1.0 → 0/6 exact**, at **1.18 → 2/6 exact** — and the failures change character, from unrecognizable ("אניבודי כתמדר", "אני רוצה כתמלך") to near-misses that differ by one letter ("אני בודק את המערכ"). Slowing the phoneme rate does not eliminate the variance but materially reduces it, so 1.18 became the shipped default. Separately, piper's `--sentence-silence` defaults to **0.0** — no pause between sentences at all — which this spike never tested and which accounted for much of the "runs together / ignores punctuation" impression; it now ships at 0.45s.
