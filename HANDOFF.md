...
- **P3 · text injection (enigo→CGEvent) needs Accessibility permission.** Even with mic fixed, dictated text won't land until the user grants **Accessibility** (separate TCC; no Info.plist key can auto-grant it). Nothing requests/checks/guides it. Recommended: `tauri-plugin-macos-permissions` (ayangweb) or a `#[cfg(target_os="macos")]` `AXIsProcessTrusted()` check + a banner that opens the Accessibility pane. `inject_text_defocused`'s 80ms hide/restore (`lib.rs:993-1042`) is also Windows-tuned.
...
**Suggested order:** ~~diarization first~~ ✅ code-done (pending one live check) → **system-audio capture next** (bigger, needs a real design pass — use the brainstorming skill before touching code, same as SRT export).
